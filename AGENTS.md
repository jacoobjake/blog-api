...
- First-time DB setup (not in the update script): copy `.env` from `.env.example`, `php artisan key:generate`, `php artisan migrate`, `php artisan db:seed`. These persist across sessions once done. The seeder only creates a test user (`SEED_TEST_USER_EMAIL`, default `test@example.com`) — it does **not** create any blogs.
...
### RBAC (Tier B Phase 1)
- Roles via **Spatie Permission**: `superadmin`, `admin`, `editor`, `author`. Seeded users: `SEED_SUPERADMIN_EMAIL` (superadmin), `SEED_TEST_USER_EMAIL` (author). Defaults: `superadmin@example.com`, `test@example.com`.
